...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.LAIA_ARCH_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.LAIA_ARCH_AUTH_STORE_READONLY;
        } else {
          process.env.LAIA_ARCH_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
