...
describe("resolveOpenClawAgentDir", () => {
  ...
  it("honors LAIA_ARCH_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          LAIA_ARCH_STATE_DIR: undefined,
          LAIA_ARCH_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveOpenClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when LAIA_ARCH_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers LAIA_ARCH_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          LAIA_ARCH_STATE_DIR: undefined,
          LAIA_ARCH_AGENT_DIR: primaryOverride,
