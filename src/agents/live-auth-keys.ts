...
const GOOGLE_LIVE_SINGLE_KEY = "LAIA_ARCH_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "LAIA_ARCH_LIVE_ANTHROPIC_KEY",
    listVar: "LAIA_ARCH_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "LAIA_ARCH_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `LAIA_ARCH_LIVE_${base}_KEY`;
