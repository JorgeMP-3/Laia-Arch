...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      LAIA_ARCH_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when LAIA_ARCH_THEME=light", async () => {
    const mod = await importThemeWithEnv({ LAIA_ARCH_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when LAIA_ARCH_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ LAIA_ARCH_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats LAIA_ARCH_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ LAIA_ARCH_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("LAIA_ARCH_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      LAIA_ARCH_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ LAIA_ARCH_THEME: "light" });
    const darkMod = await importThemeWithEnv({ LAIA_ARCH_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.LAIA_ARCH_THEME = "light";
