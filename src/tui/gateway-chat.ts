...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set LAIA_ARCH_GATEWAY_TOKEN/LAIA_ARCH_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.LAIA_ARCH_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.LAIA_ARCH_GATEWAY_PASSWORD);
