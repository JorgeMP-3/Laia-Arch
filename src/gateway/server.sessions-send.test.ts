...
const { createLaiaArchTools } = await import("../agents/laia-arch-tools.js");
...
type SessionSendTool = ReturnType<typeof createLaiaArchTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createLaiaArchTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["LAIA_ARCH_GATEWAY_PORT", "LAIA_ARCH_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.LAIA_ARCH_GATEWAY_PORT = String(gatewayPort);
  process.env.LAIA_ARCH_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("LAIA_ARCH_CONFIG_PATH missing in gateway test environment");
      }
