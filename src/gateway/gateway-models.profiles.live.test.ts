...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.LAIA_ARCH_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.LAIA_ARCH_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.LAIA_ARCH_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.LAIA_ARCH_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.LAIA_ARCH_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.LAIA_ARCH_LIVE_MAX_MODELS, 0));
}
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    agentDir: process.env.LAIA_ARCH_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.LAIA_ARCH_STATE_DIR,
  };
  ...
  process.env.LAIA_ARCH_STATE_DIR = tempStateDir;
  ...
  process.env.LAIA_ARCH_AGENT_DIR = tempAgentDir;
  ...
  try {
    ...
    process.env.LAIA_ARCH_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.LAIA_ARCH_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.LAIA_ARCH_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via LAIA_ARCH_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
