...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.LAIA_ARCH_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.LAIA_ARCH_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set LAIA_ARCH_ALLOW_INSECURE_PRIVATE_WS=1",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__LAIA_ARCH_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
