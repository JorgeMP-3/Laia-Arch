  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (LAIA_ARCH_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.LAIA_ARCH_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.LAIA_ARCH_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (LAIA_ARCH_SKIP_CHANNELS=1 or LAIA_ARCH_SKIP_PROVIDERS=1)",
        );
      } else {
