...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers LAIA_ARCH_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.LAIA_ARCH_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.LAIA_ARCH_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getHandshakeTimeoutMs()).toBe(75);
        process.env.LAIA_ARCH_HANDSHAKE_TIMEOUT_MS = "";
        expect(getHandshakeTimeoutMs()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.LAIA_ARCH_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.LAIA_ARCH_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.LAIA_ARCH_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            LAIA_ARCH_VERSION: " ",
            LAIA_ARCH_SERVICE_VERSION: "2.4.6-service",
