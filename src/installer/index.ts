...
import { createProvisionalGateway, type ProvisionalGateway } from "./provisional-gateway.js";
...
class InstallerExitError extends Error {
  constructor(public readonly code: number) {
    super(`installer exited with code ${code}`);
    this.name = "InstallerExitError";
  }
}

function abortInstaller(code: number): never {
  throw new InstallerExitError(code);
}
...
export function parseInstallerConfirmationInput(input: string): boolean | undefined {
  const norm = input.toLowerCase().trim();
  if (["s", "si", "sí", "y", "yes"].includes(norm)) {
    return true;
  }
  if (["n", "no"].includes(norm)) {
    return false;
  }
  return undefined;
}
  ...
  return new Promise((resolve) => {
    ...
    const ask = () => {
      rl.question(`  ${question} (s/n): `, (answer) => {
        const parsed = parseInstallerConfirmationInput(answer);
        if (parsed === undefined) {
          console.log(`  ${t.warn("Entrada no válida. Usa 's' para sí o 'n' para no.")}`);
          ask();
          return;
        }
        rl.close();
...
export async function runInstaller(): Promise<void> {
  ...
  let provisionalGateway: ProvisionalGateway | undefined;
  ...
  try {
    ...
    try {
      ...
      provisionalGateway = createProvisionalGateway(bootstrapResult);
      await provisionalGateway.start();
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.log("\n" + t.bad("Error configurando el proveedor IA"));
      console.log("  " + t.dim(msg));
      console.log(t.dim("\n  Ejecuta laia-arch install para intentarlo de nuevo.\n"));
      abortInstaller(1);
    }
    ...
    try {
      const conversation = await (
        ...
        ) => ReturnType<typeof runConversation>
      )(bootstrapResult, systemScan, installMode, provisionalGateway);
    ...
    await provisionalGateway?.close("installer handoff before agora-03");
    provisionalGateway = undefined;
    ...
    if (err instanceof InstallerExitError) {
      process.exit(err.code);
    }
    throw err;
  } finally {
    await provisionalGateway?.close("installer cleanup after failure");
  }
}
