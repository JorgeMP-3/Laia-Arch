...
describe("usage-format", () => {
  const originalAgentDir = process.env.LAIA_ARCH_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-usage-format-"));
    process.env.LAIA_ARCH_AGENT_DIR = agentDir;
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  });
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      delete process.env.LAIA_ARCH_AGENT_DIR;
    } else {
      process.env.LAIA_ARCH_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
