...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.LAIA_ARCH_GATEWAY_PORT;
    return;
  }
  process.env.LAIA_ARCH_GATEWAY_PORT = prevGatewayPort;
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.LAIA_ARCH_GATEWAY_PORT;
  process.env.LAIA_ARCH_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.LAIA_ARCH_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.LAIA_ARCH_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.LAIA_ARCH_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.LAIA_ARCH_GATEWAY_PASSWORD;
  } else {
    process.env.LAIA_ARCH_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
