...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["LAIA_ARCH_PROFILE"]);
    process.env.LAIA_ARCH_PROFILE = "isolated";
  });
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect LAIA_ARCH_PROFILE when set (most tests run with isolated profile)
