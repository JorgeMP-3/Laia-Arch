...
import { LAIA_ARCH_CLI_ENV_VALUE } from "../infra/laia-arch-exec-env.js";
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        LAIA_ARCH_BASE_ENV: "base",
        LAIA_ARCH_TO_REMOVE: undefined,
      },
      env: {
        LAIA_ARCH_TEST_ENV: "ok",
      },
    });

    expect(resolved.LAIA_ARCH_BASE_ENV).toBe("base");
    expect(resolved.LAIA_ARCH_TEST_ENV).toBe("ok");
    expect(resolved.LAIA_ARCH_TO_REMOVE).toBeUndefined();
    expect(resolved.LAIA_ARCH_CLI).toBe(LAIA_ARCH_CLI_ENV_VALUE);
  });
