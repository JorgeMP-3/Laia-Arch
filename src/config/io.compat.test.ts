...
describe("config io paths", () => {
  ...
  it("uses LAIA_ARCH_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { LAIA_ARCH_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
  ...
  it("honors explicit LAIA_ARCH_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".openclaw", 20002, "custom.json");
      const io = createIoForHome(home, { LAIA_ARCH_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
