...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ LAIA_ARCH_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from LAIA_ARCH_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { LAIA_ARCH_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when LAIA_ARCH_STATE_DIR differs from stored paths", () => {
