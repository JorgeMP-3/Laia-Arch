...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LAIA_ARCH_GATEWAY_TOKEN: "test-token",
    LAIA_ARCH_CONFIG_DIR: join(sandbox.rootDir, "config"),
    LAIA_ARCH_WORKSPACE_DIR: join(sandbox.rootDir, "openclaw"),
  };
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    LAIA_ARCH_CONFIG_DIR: configDir,
    LAIA_ARCH_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      LAIA_ARCH_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      LAIA_ARCH_EXTRA_MOUNTS: undefined,
      LAIA_ARCH_HOME_VOLUME: "openclaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LAIA_ARCH_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("LAIA_ARCH_EXTRA_MOUNTS=");
    expect(envFile).toContain("LAIA_ARCH_HOME_VOLUME=openclaw-home"); // pragma: allowlist secret
    ...
    expect(log).toContain("--build-arg LAIA_ARCH_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  ...
  it("writes LAIA_ARCH_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LAIA_ARCH_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LAIA_ARCH_TZ=Asia/Shanghai");
  });
  ...
  it("reuses existing config token when LAIA_ARCH_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("LAIA_ARCH_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when LAIA_ARCH_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "LAIA_ARCH_GATEWAY_TOKEN=dotenv-token-123\nLAIA_ARCH_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("LAIA_ARCH_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "LAIA_ARCH_GATEWAY_TOKEN=first-token",
        "LAIA_ARCH_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("LAIA_ARCH_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("LAIA_ARCH_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats LAIA_ARCH_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LAIA_ARCH_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg LAIA_ARCH_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg LAIA_ARCH_INSTALL_DOCKER_CLI=1");
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        LAIA_ARCH_SANDBOX: "1",
        LAIA_ARCH_DOCKER_SOCKET: socketPath,
  ...
  it("rejects injected multiline LAIA_ARCH_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LAIA_ARCH_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("LAIA_ARCH_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid LAIA_ARCH_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LAIA_ARCH_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid LAIA_ARCH_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LAIA_ARCH_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("LAIA_ARCH_HOME_VOLUME must match");
  });

  it("rejects LAIA_ARCH_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LAIA_ARCH_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("LAIA_ARCH_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/LAIA_ARCH_GATEWAY_TOKEN: \$\{LAIA_ARCH_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{LAIA_ARCH_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
