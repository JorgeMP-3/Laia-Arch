...
type HomeEnvSnapshot = {
  ...
  LAIA_ARCH_STATE_DIR: string | undefined;
  LAIA_ARCH_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    LAIA_ARCH_AGENT_DIR: process.env.LAIA_ARCH_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.LAIA_ARCH_STATE_DIR = path.join(home, ".openclaw");
    process.env.LAIA_ARCH_AGENT_DIR = path.join(home, ".openclaw", "agent");
