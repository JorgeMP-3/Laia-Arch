...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.LAIA_ARCH_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("LAIA_ARCH_HOME", snapshot.openclawHome);
  restoreKey("LAIA_ARCH_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.LAIA_ARCH_STATE_DIR = join(home, ".openclaw");
