...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      LAIA_ARCH_GATEWAY_PORT: String(params.port),
      ...(params.token ? { LAIA_ARCH_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    envSnapshot = captureEnv([
      ...
      "LAIA_ARCH_GATEWAY_PORT",
      "LAIA_ARCH_PROFILE",
    ]);
    process.env.LAIA_ARCH_STATE_DIR = "/tmp/openclaw-cli-state";
    process.env.LAIA_ARCH_CONFIG_PATH = "/tmp/openclaw-cli-state/openclaw.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        LAIA_ARCH_PROFILE: "dev",
        LAIA_ARCH_STATE_DIR: "/tmp/openclaw-daemon-state",
        LAIA_ARCH_CONFIG_PATH: "/tmp/openclaw-daemon-state/openclaw.json",
