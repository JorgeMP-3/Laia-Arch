...
export const DEFAULT_CLI_NAME = "laia-arch";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME, "openclaw"]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(laia-arch|openclaw)\b/;
...
export function resolveCliName(argv: string[] = process.argv): string {
  ...
  // Strip extension (.mjs, .js, .ts) so "laia-arch.mjs" resolves to "laia-arch"
  const base = path.basename(argv1, path.extname(argv1)).trim();
