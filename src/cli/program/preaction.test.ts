...
beforeEach(() => {
  ...
  originalHideBanner = process.env.LAIA_ARCH_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.LAIA_ARCH_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.LAIA_ARCH_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    ...
    vi.clearAllMocks();
    process.env.LAIA_ARCH_HIDE_BANNER = "1";
