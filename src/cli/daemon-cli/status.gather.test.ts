...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    LAIA_ARCH_STATE_DIR: "/tmp/openclaw-daemon",
    LAIA_ARCH_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.LAIA_ARCH_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.LAIA_ARCH_CONFIG_PATH ?? `${stateDir}/openclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.LAIA_ARCH_STATE_DIR = "/tmp/openclaw-cli";
    process.env.LAIA_ARCH_CONFIG_PATH = "/tmp/openclaw-cli/openclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        LAIA_ARCH_GATEWAY_PORT: "19001",
        LAIA_ARCH_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
        LAIA_ARCH_STATE_DIR: "/tmp/openclaw-daemon",
      } as Record<string, string>,
    });
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.LAIA_ARCH_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.LAIA_ARCH_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.LAIA_ARCH_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
