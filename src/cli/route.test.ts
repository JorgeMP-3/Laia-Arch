...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.LAIA_ARCH_DISABLE_ROUTE_FIRST;
    delete process.env.LAIA_ARCH_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.LAIA_ARCH_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });
