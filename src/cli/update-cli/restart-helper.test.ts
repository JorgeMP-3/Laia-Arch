...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses LAIA_ARCH_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        LAIA_ARCH_PROFILE: "default",
        LAIA_ARCH_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses LAIA_ARCH_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        LAIA_ARCH_PROFILE: "default",
        LAIA_ARCH_LAUNCHD_LABEL: "com.custom.openclaw",
      });
    ...
    it("uses LAIA_ARCH_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        LAIA_ARCH_PROFILE: "default",
        LAIA_ARCH_WINDOWS_TASK_NAME: "OpenClaw Gateway (custom)",
      });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        LAIA_ARCH_LAUNCHD_LABEL: "ai.openclaw.it's-a-test",
      });
