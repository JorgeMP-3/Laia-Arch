...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { LAIA_ARCH_PROFILE: "work", LAIA_ARCH_SERVICE_VERSION: "1.0.0" },
        description: "Custom",
      }),
    ).toBe("Custom");
  });
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { LAIA_ARCH_PROFILE: "work", LAIA_ARCH_SERVICE_VERSION: "local" },
        environment: { LAIA_ARCH_SERVICE_VERSION: "remote" },
      }),
