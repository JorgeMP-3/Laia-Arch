...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds LAIA_ARCH_GATEWAY_TOKEN and should be reinstalled.",
  ...
  issues.push({
    ...
    message:
      "Gateway service LAIA_ARCH_GATEWAY_TOKEN does not match gateway.auth.token in openclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.LAIA_ARCH_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.LAIA_ARCH_GATEWAY_TOKEN?.trim() || undefined;
}
