...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.LAIA_ARCH_GATEWAY_PORT).toBe("18789");
    expect(env.LAIA_ARCH_GATEWAY_TOKEN).toBeUndefined();
    expect(env.LAIA_ARCH_SERVICE_MARKER).toBe("openclaw");
    expect(env.LAIA_ARCH_SERVICE_KIND).toBe("gateway");
    expect(typeof env.LAIA_ARCH_SERVICE_VERSION).toBe("string");
    expect(env.LAIA_ARCH_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    expect(env.LAIA_ARCH_WINDOWS_TASK_NAME).toBe("OpenClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.LAIA_ARCH_LAUNCHD_LABEL).toBe("ai.openclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", LAIA_ARCH_PROFILE: "work" },
      port: 18789,
    });
    expect(env.LAIA_ARCH_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    expect(env.LAIA_ARCH_WINDOWS_TASK_NAME).toBe("OpenClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.LAIA_ARCH_LAUNCHD_LABEL).toBe("ai.openclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through LAIA_ARCH_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", LAIA_ARCH_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.LAIA_ARCH_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to LAIA_ARCH_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.LAIA_ARCH_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers LAIA_ARCH_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        LAIA_ARCH_GATEWAY_TOKEN: "openclaw-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.LAIA_ARCH_GATEWAY_TOKEN).toBe("openclaw-token");
  });

  it("omits LAIA_ARCH_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  ...
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", LAIA_ARCH_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", LAIA_ARCH_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw"));
  });

  it("uses LAIA_ARCH_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", LAIA_ARCH_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in LAIA_ARCH_STATE_DIR", () => {
    const env = { HOME: "/Users/test", LAIA_ARCH_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { LAIA_ARCH_STATE_DIR: "C:\\State\\openclaw" };
