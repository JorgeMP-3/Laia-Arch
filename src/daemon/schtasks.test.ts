...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when LAIA_ARCH_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".openclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when LAIA_ARCH_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", LAIA_ARCH_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".openclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers LAIA_ARCH_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        LAIA_ARCH_PROFILE: "rescue",
        LAIA_ARCH_STATE_DIR: "C:\\State\\openclaw",
      },
      expected: path.join("C:\\State\\openclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", LAIA_ARCH_PROFILE: "default" },
...
describe("readScheduledTaskCommand", () => {
  ...
  it("reads script from LAIA_ARCH_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ LAIA_ARCH_STATE_DIR: path.join(tmpDir, "custom-state") }),
