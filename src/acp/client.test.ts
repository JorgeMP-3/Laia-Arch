...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets LAIA_ARCH_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openclaw",
    });

    expect(env.LAIA_ARCH_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openclaw");
  });

  it("overrides pre-existing LAIA_ARCH_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      LAIA_ARCH_SHELL: "wrong",
    });
    expect(env.LAIA_ARCH_SHELL).toBe("acp-client");
  });
  ...
  it("preserves LAIA_ARCH_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        LAIA_ARCH_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["LAIA_ARCH_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OpenClaw bridge", () => {
    ...
    expect(env.LAIA_ARCH_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.LAIA_ARCH_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("LAIA_ARCH_API_KEY")).toBe(false);
  });
});
