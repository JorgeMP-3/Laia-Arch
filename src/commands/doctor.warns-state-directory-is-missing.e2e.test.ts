...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    process.env.LAIA_ARCH_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when LAIA_ARCH_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.LAIA_ARCH_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.LAIA_ARCH_GATEWAY_TOKEN;
    delete process.env.LAIA_ARCH_GATEWAY_TOKEN;
    note.mockClear();
    try {
      await doctorCommand(createDoctorRuntime(), {
      ...
      if (previousToken === undefined) {
        ...
        process.env.LAIA_ARCH_GATEWAY_TOKEN = previousToken;
      }
    }
