...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.LAIA_ARCH_STATE_DIR = stateDir;
  ...
  it("uses LAIA_ARCH_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.LAIA_ARCH_GATEWAY_TOKEN = envToken;
        ...
        await runNonInteractiveSetup(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "LAIA_ARCH_GATEWAY_TOKEN",
          },
          runtime,
        );
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.LAIA_ARCH_STATE_DIR = stateDir;
      process.env.LAIA_ARCH_CONFIG_PATH = path.join(stateDir, "openclaw.json");
