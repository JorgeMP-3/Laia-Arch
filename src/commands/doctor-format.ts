...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.LAIA_ARCH_PROFILE);
    hints.push(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        platform,
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.LAIA_ARCH_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.LAIA_ARCH_PROFILE),
      }),
    );
  }
  return hints;
