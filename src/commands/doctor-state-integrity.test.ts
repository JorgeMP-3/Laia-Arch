...
type EnvSnapshot = {
  HOME?: string;
  LAIA_ARCH_HOME?: string;
  LAIA_ARCH_STATE_DIR?: string;
  LAIA_ARCH_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    LAIA_ARCH_HOME: process.env.LAIA_ARCH_HOME,
    LAIA_ARCH_STATE_DIR: process.env.LAIA_ARCH_STATE_DIR,
    LAIA_ARCH_OAUTH_DIR: process.env.LAIA_ARCH_OAUTH_DIR,
  };
}
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    process.env.LAIA_ARCH_HOME = tempHome;
    process.env.LAIA_ARCH_STATE_DIR = path.join(tempHome, ".openclaw");
    delete process.env.LAIA_ARCH_OAUTH_DIR;
    fs.mkdirSync(process.env.LAIA_ARCH_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when LAIA_ARCH_OAUTH_DIR is explicitly configured", async () => {
    process.env.LAIA_ARCH_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
