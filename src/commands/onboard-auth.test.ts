...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under LAIA_ARCH_AGENT_DIR when set", async () => {
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.LAIA_ARCH_AGENT_DIR = kidAgentDir;
  ...
  it("syncs siblings from explicit agentDir outside LAIA_ARCH_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-external-"));
    process.env.LAIA_ARCH_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* LAIA_ARCH_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to LAIA_ARCH_AGENT_DIR when set", async () => {
