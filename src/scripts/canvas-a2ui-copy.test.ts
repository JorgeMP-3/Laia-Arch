...
describe("canvas a2ui copy", () => {
  ...
  it("skips missing assets when LAIA_ARCH_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.LAIA_ARCH_A2UI_SKIP_MISSING;
      process.env.LAIA_ARCH_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.LAIA_ARCH_A2UI_SKIP_MISSING;
        } else {
          process.env.LAIA_ARCH_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
