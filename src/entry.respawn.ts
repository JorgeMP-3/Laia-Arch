...
export const LAIA_ARCH_NODE_OPTIONS_READY = "LAIA_ARCH_NODE_OPTIONS_READY";
export const LAIA_ARCH_NODE_EXTRA_CA_CERTS_READY = "LAIA_ARCH_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.LAIA_ARCH_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[LAIA_ARCH_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[LAIA_ARCH_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[LAIA_ARCH_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[LAIA_ARCH_NODE_OPTIONS_READY] = "1";
