...
const LAIA_ARCH_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of LAIA_ARCH_COMPILED_SECRET_TARGETS) {
...
const LAIA_ARCH_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: LAIA_ARCH_COMPILED_SECRET_TARGETS,
    entriesById: LAIA_ARCH_TARGETS_BY_ID,
  });
