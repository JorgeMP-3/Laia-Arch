...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.LAIA_ARCH_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.LAIA_ARCH_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.LAIA_ARCH_GATEWAY_TOKEN) ||
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set LAIA_ARCH_GATEWAY_TOKEN/LAIA_ARCH_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openclaw security audit --deep")}".`,
    });
  }
