...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.LAIA_ARCH_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.LAIA_ARCH_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set LAIA_ARCH_APNS_TEAM_ID and LAIA_ARCH_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.LAIA_ARCH_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.LAIA_ARCH_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.LAIA_ARCH_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set LAIA_ARCH_APNS_PRIVATE_KEY_P8 or LAIA_ARCH_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading LAIA_ARCH_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
