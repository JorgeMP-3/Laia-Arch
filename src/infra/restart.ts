...
export function triggerOpenClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.LAIA_ARCH_SYSTEMD_UNIT,
      process.env.LAIA_ARCH_PROFILE,
    );
  ...
  const label =
    process.env.LAIA_ARCH_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.LAIA_ARCH_PROFILE);
