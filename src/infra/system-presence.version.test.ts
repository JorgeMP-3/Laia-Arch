...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when LAIA_ARCH_VERSION is not set", async () => {
  ...
  it("prefers LAIA_ARCH_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over LAIA_ARCH_SERVICE_VERSION when LAIA_ARCH_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        LAIA_ARCH_VERSION: " ",
        LAIA_ARCH_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when LAIA_ARCH_VERSION and LAIA_ARCH_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        LAIA_ARCH_VERSION: " ",
        LAIA_ARCH_SERVICE_VERSION: "\t",
