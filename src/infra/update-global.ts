...
export const LAIA_ARCH_MAIN_PACKAGE_SPEC = "github:openclaw/openclaw#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Laia Arch", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.LAIA_ARCH_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.LAIA_ARCH_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return LAIA_ARCH_MAIN_PACKAGE_SPEC;
  }
