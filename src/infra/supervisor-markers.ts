const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "LAIA_ARCH_LAUNCHD_LABEL"],
  systemd: ["LAIA_ARCH_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["LAIA_ARCH_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "LAIA_ARCH_SERVICE_MARKER",
  "LAIA_ARCH_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.LAIA_ARCH_SERVICE_MARKER?.trim();
    const serviceKind = env.LAIA_ARCH_SERVICE_KIND?.trim();
